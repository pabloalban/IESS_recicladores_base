...
rendimiento_biess <- read_excel ( 
  ...
  mutate ( fecha = as.Date ( fecha, "%Y-%m-%d", tz = parametros$time_zone ) ) %>%
  filter ( year ( fecha ) <= anio_fin )
...
pobreza_extrema <- read_excel ( 
  file,
  sheet = 'tasa_pobreza_extrema',
  col_names = TRUE,
  col_types = NULL,
  na = "",
  skip = 0
