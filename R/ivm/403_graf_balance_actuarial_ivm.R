...
for ( i in 1:length( escenarios_lista ) ) { # i <- 1
  ...
  x_lim <- c( parametros$anio_ini, parametros$anio_ini + parametros$ivm_horizonte )
  ...
  plt_bal_act <- ggplot() +
    geom_line( data = balance_anual, aes( x = t, y = V_cap ), 
               linewidth = graf_line_size, color = parametros$iess_blue ) +
    geom_hline( aes( yintercept = 0 ), linewidth = 0.5 * graf_line_size,
...
for ( i in 1:length( escenarios_lista ) ) {
  ...
  ggsave( plot = plt_bal_act, 
          filename = paste0( parametros$resultado_graficos, 'iess_balance_vap_',
                             escenario, parametros$graf_ext ),
          width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
  ...
  ggsave( plot = plt_bal_act, 
          filename = paste0( parametros$resultado_graficos, 'iess_balance_vap_rv_', 
                             escenario, parametros$graf_ext ),
          width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
  ...
  ggsave( plot = plt_apo_ben_act, 
          filename = paste0( parametros$resultado_graficos, 'iess_apo_ben_bal_dinamico_',
                             escenario, parametros$graf_ext ),
          width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
}
